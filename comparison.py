...
class Comparison:
    ...
    def compare(self, item_to_compare):
        ...
        print("tutaj")
        type(list_of_objects[0].list_items[0])
        print(list_of_objects[0].list_items)

        for x in list_of_objects:

            if ('unit_price') in list_of_objects[0].list_items:
                x.list_items.sort(key=sort_by_price)
            else:
                print(x.list_items)
                print("uzyto sortowania buyout")
    ...
    def make_avg_percent_price(self, valuepercent):
        ...
        for object_list in self.object_from_compare:
            ap = mathprice.avg_percent_items_price(101, object_list.list_items)
